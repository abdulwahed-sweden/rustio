...
///
///   ALTER TABLE child ADD COLUMN via BIGINT REFERENCES parent(id)
///       ON DELETE <policy>;
///
/// PostgreSQL enforces foreign keys by default — no PRAGMA needed.
/// The column is nullable: a `required: true` relation can't be
/// added via `ALTER TABLE … ADD COLUMN` on a populated table without
/// a default (PG would raise "column contains null values"). The
/// retrofit command (`rustio migrate --add-fks`) handles the
/// add-nullable / backfill / SET NOT NULL sequence in three statements.
///
...
fn apply_add_relation(
    ...
    // Reject NOT NULL + REFERENCES on `ai apply`: PG would raise
    // "column contains null values" on any non-empty table because
    // ADD COLUMN with a `NOT NULL` constraint requires either an
    // empty table or a `DEFAULT`. The retrofit command sequences
    // add-nullable / backfill / SET NOT NULL into three statements.
    if r.required {
        return Err(ExecutionError::UnsupportedPrimitive {
            op: "add_relation",
...
/// Phase 2: emit the SQL for adding a FK column to an existing PG
/// table. PostgreSQL enforces foreign keys by default, so the SQLite
/// `PRAGMA foreign_keys = ON` is gone. Column type is `BIGINT` to
/// match the parent's `BIGSERIAL PRIMARY KEY`.
...
fn apply_remove_field(
    ...
    // Patch the models.rs file (struct field, COLUMNS array, from_row,
    // insert_values). Same machinery whether we're dropping a plain
    // field or a relation FK column — only the SQL emission differs.
    let patched =
    ...
    // Phase 2: PostgreSQL has a real `ALTER TABLE … DROP COLUMN`. The
    // SQLite recreate-table dance is gone. CASCADE drops any FK or
    // index that depended on the column — exactly what a remove_field
    // operation should do (the user accepted the destructive flag).
    let sql = format!(
        "-- Generated by rustio ai apply. DO NOT EDIT.\n\
...
fn apply_change_field_type(
    ...
    // Phase 2: PostgreSQL has native column-type alteration. The SQLite
    // FK guard is gone (PG re-validates dependent FKs as part of the
    // ALTER, refusing the migration if a stored value would violate
    // them — exactly the behaviour the SQLite recreate-table dance was
    // approximating). We still patch the Rust source so the in-tree
    // model matches the new column type.
...
/// Phase 2: decide whether `old_ty` → `new_ty` is a safe PostgreSQL cast
/// and return the expression that goes inside the `USING (…)` clause of
/// `ALTER TABLE … ALTER COLUMN … TYPE …`. `None` means "not in the
/// safe-cast set" — callers refuse with `UnsupportedPrimitive`.
///
/// PG, unlike SQLite, has real types and won't auto-coerce between
/// `INTEGER` ↔ `BOOLEAN` ↔ `TEXT` ↔ `TIMESTAMPTZ`. Every cross-type
/// change needs an explicit `::target` cast. Failures (a non-numeric
/// `TEXT` → `BIGINT`, a malformed timestamp string) surface at apply
/// time as a Postgres error, not as silent data loss.
fn cast_expression(old_ty: &str, new_ty: &str, col_name: &str) -> Option<String> {
    match (old_ty, new_ty) {
        (a, b) if a == b => None,
        // INTEGER ↔ BIGINT — PG won't widen implicitly inside USING.
        ("i32", "i64") => Some(format!("{col_name}::BIGINT")),
        ("i64", "i32") => Some(format!("{col_name}::INTEGER")),
        // BOOLEAN ↔ INTEGER / BIGINT — PG provides explicit casts.
        ("bool", "i32") => Some(format!("{col_name}::INTEGER")),
        ("bool", "i64") => Some(format!("{col_name}::BIGINT")),
        ("i32", "bool") | ("i64", "bool") => Some(format!("{col_name}::BOOLEAN")),
        // TIMESTAMPTZ ↔ TEXT — both directions need an explicit cast.
        ("DateTime", "String") => Some(format!("{col_name}::TEXT")),
        ("String", "DateTime") => Some(format!("{col_name}::TIMESTAMPTZ")),
        // Widening to TEXT is always safe.
        ("i32", "String") | ("i64", "String") | ("bool", "String") => {
            Some(format!("{col_name}::TEXT"))
        }
        // Narrowing from TEXT — PG raises a runtime error if a row's
        // value isn't parseable. Reviewer accepts that risk.
...
fn apply_change_field_nullability(
    ...
    // Phase 2: PostgreSQL has native nullability flips. The SQLite
    // FK guard is gone — PG's `ALTER COLUMN SET/DROP NOT NULL` doesn't
    // disturb FKs at all.
    ...
    let sql = if tightening {
        // Backfill any existing NULLs with the type's safe default
        // first, then flip the constraint. The two statements run in
        // the same migration; PG executes them sequentially.
...
fn apply_rename_model(
    ...
    // Phase 2: PG's ALTER TABLE … RENAME TO automatically updates
    // every dependent FK reference, so the SQLite FK guard is gone.
...
// ---------------------------------------------------------------------------
//
// The SQLite recreate-table machinery (generate_sqlite_recreate_table_migration,
// generate_sqlite_recreate_table_migration_fk_aware, column_def,
// column_def_with_relation_context, table_has_any_foreign_key) is gone.
// PostgreSQL's native ALTER paths replaced every caller —
// apply_change_field_type, apply_change_field_nullability,
// apply_remove_field, apply_remove_relation, apply_rename_model, and
// plan_retrofit_foreign_keys. See the per-function comments for
// the new SQL shape each one emits.
...
/// Phase 2: generate a retrofit plan for every belongs_to relation
/// that lacks `on_delete` metadata. Returns a report the caller can
/// either print (dry-run) or materialise (write migrations + update
/// schema).
///
/// Postgres rewrite — no recreate-table dance. Each missing FK becomes
/// a single `ALTER TABLE … ADD CONSTRAINT … FOREIGN KEY … REFERENCES …
/// ON DELETE …`. PG validates the existing rows during the ALTER:
/// the migration refuses cleanly if any child row's FK column points
/// at a non-existent parent (operators clean orphans before retry).
///
/// One migration file per model that needs retrofitting; each file
/// contains one ALTER per upgraded relation, wrapped in BEGIN/COMMIT
/// for atomicity.
pub fn plan_retrofit_foreign_keys(schema: &crate::schema::Schema) -> RetrofitReport {
    ...
    for model in &schema.models {
        ...
        // Collect the relations on this model that need retrofitting.
        let mut to_retrofit: Vec<(String, String, String)> = Vec::new(); // (via, parent_table, policy)
        for f in &model.fields {
            if let Some(rel) = &f.relation {
                if matches!(rel.kind, RelationKind::BelongsTo) && rel.on_delete.is_none() {
                    ...
                    let policy = "RESTRICT".to_string(); // retrofit default
                    to_retrofit.push((f.name.clone(), parent_table, policy));
        ...
        // PG migration: one ALTER TABLE ADD CONSTRAINT per relation,
        // wrapped in BEGIN/COMMIT so the whole batch is atomic.
