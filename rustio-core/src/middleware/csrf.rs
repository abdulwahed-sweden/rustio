...
//! and works with HTML forms and JS clients alike. Templates can read
//! the cookie server-side through `Identity`-like plumbing, but the
//! easiest path is to inject the token via a handler helper.
...
pub async fn csrf_protect(mut req: Request, next: Next) -> Result<Response> {
    ...
    req.ctx_mut().insert(CsrfGuard { token: token.clone() });
    ...
    if !is_safe(req.method()) {
        let provided = req
            .header(CSRF_HEADER)
            .map(|s| s.to_string())
            .or_else(|| req.form().ok().and_then(|f| f.get(CSRF_FIELD).map(|v| v.to_string())));
