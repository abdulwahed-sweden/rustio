//! Versioned SQL migrations for PostgreSQL. Transactional where
//! possible; the tracking table records which versions have been
//! applied.
...
pub async fn applied_versions(db: &Db) -> Result<Vec<i64>> {
    ...
    let rows = sqlx::query_scalar::<_, i64>(
        "SELECT version FROM rustio_migrations ORDER BY version ASC",
    )
    .fetch_all(db.pool())
    .await?;
    Ok(rows)
...
fn split_statements(sql: &str) -> Vec<String> {
    ...
    while let Some(c) = chars.next() {
        ...
        match c {
            ...
            '$' => {
                // Potential dollar-quoted body: $tag$...$tag$
                ...
                if tag.ends_with('$') && tag.len() >= 2 {
                    // Consume the characters we peeked.
                    for _ in 1..tag.len() {
                        current.push(chars.next().unwrap());
                    }
                    current.insert(current.len() - tag.len() + 1, '$');
                    // actually push the opening $
                    current.push('$');
                    dollar_tag = tag;
                    in_dollar = true;
                } else {
