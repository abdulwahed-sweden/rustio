...
pub fn derive_suggestions_from_entries(
    ...
    // TODO(phase-5-followup): real implementation is deferred until the
    // entry_builder module is fully ported from OLD. The stub returns an
    // empty Vec so callers compile and behave as "no suggestions" — same
    // outcome as the no-context branch of derive_suggestions. Unblocks
    // once DynamicAdminEntry gains its real shape + build_admin_entries
    // reads a live Schema.
    Vec::new()
}
...
pub fn find_suggestion_from_entries(
    ...
    // TODO(phase-5-followup): stubbed for the same reason as
    // derive_suggestions_from_entries — returns None until entry_builder
    // lands.
    None
}
