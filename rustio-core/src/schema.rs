...
impl SchemaField {
    fn from_admin_field(f: &AdminField) -> Self {
        ...
        // a struct field is the only writer the admin blessed. NEW's
        // `AdminRelation` is slimmer than OLD's — only `target_model`
        // and `display_field`. The relation kind is implicitly
        // `BelongsTo` (NEW doesn't model HasMany at the field level).
        ...
        Self {
            ...
            // NEW's `AdminField.field_type` is a `FieldType` enum; OLD's
            // had a separate `nullable` bool. NEW encodes nullability in
            // `OptionalI64` / `OptionalString` variants, so the bool
            // comes back via the enum's `nullable()` accessor.
...
pub(crate) fn field_type_name(ty: FieldType) -> &'static str {
    // Nullability is captured separately in `SchemaField.nullable`, so
    // the optional variants map to their non-optional name. The two
    // shapes round-trip through `(ty, nullable)` faithfully.
    match ty {
        FieldType::I32 => "i32",
        FieldType::I64 => "i64",
